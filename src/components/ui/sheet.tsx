...
function SheetOverlay({ className, ...props }: SheetPrimitive.Backdrop.Props) {
 ...
 className={[
 "fixed inset-0 z-50 bg-black/10 transition-opacity data-ending-style:opacity-0 data-starting-style:opacity-0 supports-backdrop-filter:backdrop-blur-xs",
 className
 ].filter(Boolean).join(' ')}
 {...props}
 />
...
function SheetHeader({ className, ...props }: React.ComponentProps<"div">) {
 ...
 className={["flex flex-col gap-0.5 p-4", className].filter(Boolean).join(' ')}
 {...props}
 />
 )
}
...
function SheetFooter({ className, ...props }: React.ComponentProps<"div">) {
 ...
 className={["mt-auto flex flex-col gap-2 p-4", className].filter(Boolean).join(' ')}
 {...props}
 />
 )
}
