        ...
        {transaction && (
          <div className="flex-1 px-6 pb-8 flex flex-col h-full overflow-y-auto">
            <div className="flex justify-between items-center mb-6 border-b-4 border-black pb-2">
              <h2 className="text-2xl font-black tracking-widest">{t('ledger_edit.title')}</h2>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
              <div className="bg-zinc-100 p-4 border-4 border-black mb-6">
                ...
                <span className="block font-black text-xl overflow-hidden text-ellipsis whitespace-nowrap">{transaction.description || t('ledger.no_description')}</span>
              ...
              <div className="space-y-2">
                <label className="font-bold text-sm tracking-wider">{t('ledger_edit.category')}</label>
                <input 
                  {...register('category')} 
              ...
              <div className="space-y-2">
                <label className="font-bold text-sm tracking-wider">{t('ledger_edit.status')}</label>
                <input 
                  {...register('status')} 
                  className="w-full border-2 border-black px-4 py-3 font-bold text-lg focus:outline-none focus:ring-2 focus:ring-black relative z-10"
                />
              ...
              <Button type="submit" className="w-full mt-6" size="lg" disabled={updateMutation.isPending}>
                {updateMutation.isPending ? (
                  <><FontAwesomeIcon icon={faSpinner} className="animate-spin mr-2" /> {t('ledger_edit.saving')}</>
                ) : (
                  <><FontAwesomeIcon icon={faSave} className="mr-2" /> {t('ledger_edit.update_btn')}</>
                )}
              </Button>
            </form>
          </div>
