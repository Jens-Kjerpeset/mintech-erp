...
import { faStore, faReceipt, faPaperclip, faExclamationTriangle, faCloudArrowDown, faWandMagicSparkles } from '@fortawesome/free-solid-svg-icons';
...
function ValutaInput({ value, onChange, highlight }: { value: number, onChange: (val: number) => void, highlight?: boolean }) {
 ...
 type="text"
 inputMode="decimal"
 className={`font-mono ${highlight ? 'ring-2 ring-primary/50 bg-primary/10' : ''}`}
 value={localStr}
 onChange={(e) => {
...
export function ZReportBuilder({ open, onOpenChange }: { open: boolean; onOpenChange: (show: boolean) => void }) {
 ...
 title={<><FontAwesomeIcon icon={faStore} className="h-6 w-6 text-primary shrink-0" /> Nytt kasseoppgjør</>}
 ...
 {error && (
 <div className="mb-2 p-4 rounded-none bg-destructive/10 border border-destructive/20 text-destructive text-sm flex gap-2 items-start">
 ...
 <h3 className="font-medium tracking-tight text-sm tracking-wider text-muted-foreground flex items-center gap-2">
 Dato & Tidspunkt
 </h3>
 ...
 {isFetchingPOS ? <><FontAwesomeIcon icon={faWandMagicSparkles} className="w-4 h-4 " /> Henter kassedata...</> : <><FontAwesomeIcon icon={faCloudArrowDown} className="w-4 h-4" /> Hent fra kassesystem</>}
 </Button>
 </div>
 ...
 <div className="p-3 bg-muted/50 rounded-none flex items-center justify-between border">
 <span className="font-medium text-muted-foreground">Brutto omsetning</span>
 <span className="text-xl font-medium tracking-tight font-mono">{formatCurrency(grossSales)}</span>
 </div>
 </div>
 ...
 <div className={`p-3 rounded-none flex flex-col justify-center border transition-colors ${isVatBalanced ? 'bg-green-500/10 border-green-500/20 text-green-600' : 'bg-destructive/10 border-destructive/20 text-destructive'}`}>
 <div className="flex items-center justify-between">
 <span className="font-medium">Sum Fordelt</span>
 <span className="font-medium tracking-tight font-mono">{formatCurrency(totalVatInputs)}</span>
 </div>
 {!isVatBalanced && grossSales > 0 && (
 <p className="text-xs font-medium tracking-tight mt-1 text-center">MVA-registreringen stemmer ikke med brutto omsetning.</p>
 )}
 </div>
 </div>
 ...
 <div className="flex items-center justify-between p-3 border rounded-none bg-muted/30">
 <span className="font-medium text-muted-foreground">Kassedifferanse (Manko/Overskudd)</span>
 <span className={`text-xl font-medium tracking-tight font-mono ${cashDifference < 0 ? 'text-red-500' : cashDifference > 0 ? 'text-green-600 dark:text-green-500' : 'text-foreground'}`}>
 ...
 className={`w-full p-6 border-2 border-dashed rounded-none flex flex-col items-center justify-center cursor-pointer transition-colors ${draft.receiptUrl ? 'bg-primary/5 border-primary/30' : 'hover:bg-accent/50 hover:border-primary/30'}`}
 >
 {draft.receiptUrl ? (
 <>
 <div className="w-12 h-12 rounded-full bg-green-500/20 flex items-center justify-center mb-3">
 <FontAwesomeIcon icon={faReceipt} className="w-6 h-6 text-green-500" />
 </div>
 ...
 <FontAwesomeIcon icon={faPaperclip} className="w-6 h-6 text-muted-foreground" />
 </div>
